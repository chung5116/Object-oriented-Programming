...
void Hero::drawAttckRange(vector<Creature>& gCreature) {
	int hx = posHero.x;
	int hy = posHero.y;
	vector<vector<char>>attackBoard = gBoard_obj.getBoard();
	if (gBoard_obj.checkValid(hx - 1, hy)) {
		attackBoard[hx - 1][hy] = '*';		// top
	}
	if (gBoard_obj.checkValid(hx - 2, hy)) {
		attackBoard[hx - 2][hy] = '*';		// top
	}
	if (gBoard_obj.checkValid(hx + 1, hy)) {
		attackBoard[hx +1 ][hy] = '*';		// bottom
	}
	if (gBoard_obj.checkValid(hx + 2, hy)) {
		attackBoard[hx + 2][hy] = '*';		// bottom
	}

	if (gBoard_obj.checkValid(hx, hy - 1)) {
		attackBoard[hx][hy - 1] = '*';		// left
	}
	if (gBoard_obj.checkValid(hx, hy - 2)) {
		attackBoard[hx][hy - 2] = '*';		// left
	}
	if (gBoard_obj.checkValid(hx, hy + 1)) {
		attackBoard[hx][hy + 1] = '*';		//right
	}
	if (gBoard_obj.checkValid(hx, hy +2)) {
		attackBoard[hx][hy + 2] = '*';		//right
	}
	...
	for (int i = 0; i < gCreature.size(); i++) {
		if (gCreature[i].exist == true) {
			//�ͪ��bHero�������d��
			Position pos = gCreature[i].getPos();
			if (posHero.x == pos.x && posHero.y == pos.y) {
				gCreature[i].hp = gCreature[i].hp - damage;
			}
			if (attackBoard[pos.x][pos.y]=='*') {
				gCreature[i].hp = gCreature[i].hp -damage;
			}
			if (gCreature[i].hp <= 0) {
				gCreature[i].exist = false;
...
/*void Hero::heroPassTrigger(Trigger* gTrigger[3], Creature* gCreture[3]) {
	bool trig = false;    // �Ĩ�trigger �� true
	for (int i = 0; i < 3; i++) {
		if (gTrigger[i]->exist) {
