...
int main() {
	...
	while (gHero_obj.hp > 0) {
		...
		for (int i = 0; i < gNumOfItems - 1; i++) {
			gItems[i]->triggered(gHero_obj);
		}
...
void printBoard() {
	...
	for (int i = 0; i < gNumOfItems; i++) {				//if item exist,then put in the board
		if (gItems[i]->exist == true) {
			Position pos = gItems[i]->getPos();
			char icon = gItems[i]->getIcon();
	...
	for (int i = 0; i < (*tmpBoard).size(); i++) {
		for (int j = 0; j < (*tmpBoard)[i].size(); j++) {
			...
			if (print) {
				for (int k = 0; k <gnumOfCreature; k++) {
					if (gCreature[k]->exist) {
